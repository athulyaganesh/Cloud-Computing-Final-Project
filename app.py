...
def get_figures():
    ...
    figs['fig_units_by_month'] = px.line(units_month_df, x="PURCHASE_MONTH", y="UNITS", title="Units by Month",
                                         markers=True)
    ...
    figs['fig_units_by_region'] = px.pie(units_region_df, values='UNITS', names='STORE_REGION',
    ...
    figs['fig_spend_by_month'] = px.line(spend_month_df, x="PURCHASE_MONTH", y="SPEND", title="Spend by Month",
                                         markers=True)
    ...
    figs['fig_spend_by_region'] = px.pie(spend_region_df, values='SPEND', names='STORE_REGION',
    ...
    figs['fig_spend_by_marital'] = px.pie(spend_marital_df, values='SPEND', names='MARITAL',
    ...
    figs['fig_spend_by_children'] = px.pie(spend_children_df, values='SPEND', names='CHILDREN',
    ...
    figs['fig_spend_by_hshdcomposition'] = px.pie(spend_hshdcomposition_df, values='SPEND', names='HSHD_COMPOSITION',
    ...
    figs['fig_units_by_region_over_year'] = px.sunburst(units_by_region_over_year_df, path=['YEAR', 'STORE_R'],
                                                        values='UNITS', title="Units by Region")

    # spend by region over year
    figs['fig_spend_by_region_over_year'] = px.sunburst(units_by_region_over_year_df, path=['YEAR', 'STORE_R'],
                                                        values='SPEND', title="Spend By Region")
    ...
    figs['fig_units_by_dept_over_year_df'] = px.sunburst(units_by_dept_over_year_df, path=['YEAR', 'DEPARTMENT'],
                                                         values='UNITS', title="Units By Department")
    figs['fig_spend_by_dept_over_year_df'] = px.sunburst(units_by_dept_over_year_df, path=['YEAR', 'DEPARTMENT'],
...
def serve_layout():
    ...
    dashboard_layout = html.Div(children=[
        ...
        html.P([
                   'The below table combines retail data for transactions, households, and products. Each column is searchable and sortable using the second row as search input.']),
        ...
        html.P([
            ...
            html.Br(),
            html.A(href='https://colab.research.google.com/drive/1AZ-o4M57UeZ0a2DAndHy7FdJ7xHYIvQh?usp=sharing',
